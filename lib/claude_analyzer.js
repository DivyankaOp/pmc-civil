...
const os   = require('os');
const { ratesFilePath, symbolsLearnedPath, scriptsPath } = require('../paths');

// ── RATES from Rates.json ─────────────────────────────────────────
const RATES = (() => {
  const out = {};
  try {
    const raw = JSON.parse(fs.readFileSync(ratesFilePath(), 'utf8'));
...
function knowledgeBaseHints() {
  try {
    const kb = JSON.parse(fs.readFileSync(symbolsLearnedPath(), 'utf8'));
    const hints = [];
...
function runCVAnalysis(b64Image) {
  try {
    ...
    const result = execSync(`python3 "${scriptsPath('drawing_cv.py')}" ${tmp}`, {timeout:30000});
    fs.unlinkSync(tmp);
...
async function geminiAnalyzeDrawing(key, files, cvData, fetchFn) {
  // DISABLED 5-phase vision pipeline (was burning tokens on repeated image calls).
  // Prefer schedule-first local path via drawing_analyzer / schedule_pipeline.
  // Kept as thin single-call fallback for legacy imports only.
  console.log('\n[PMC] === Legacy 5-phase DISABLED — using single vision call fallback ===');
  const { extractSchedules } = require('./schedule_extractor');
  const { buildBoqFromSchedules } = require('./boq_from_schedules');

  const textBits = [
    cvData?.drawing_context_text,
    cvData?.gcv_raw_text,
    typeof cvData?.gcv_validated_table === 'object' ? JSON.stringify(cvData.gcv_validated_table) : '',
  ].filter(Boolean).join('\n');

  const extracted = extractSchedules(textBits);
  if (extracted.quality !== 'poor' && extracted.total_schedule_rows >= 1) {
    const local = buildBoqFromSchedules(extracted);
    console.log(`[PMC] schedule-first inside claude_analyzer: ${local.boq.length} items`);
    return {
      ...local,
      elements: local.boq.map((item, i) => ({
      ...
      pipeline_info: { phases_completed: 0, mode: 'schedule-first', model: 'local' },
      prepared_by: 'PMC Civil AI — Schedule-first (5-phase disabled)',
    };
  }

  // Last resort: ONE vision call (not 5)
  try {
    const vision = await claudeAnalyzeDrawingVision(files, '', '');
    return vision;
  } catch (e) {
    console.error('[PMC] single vision fallback failed:', e.message);
    return buildBoqFromSchedules(extracted);
  }
}
...
async function claudeAnalyzeDWGVision(pngTiles, converterResult, filename) {
  const capped = (pngTiles || []).slice(0, 3);
  console.log('[claudeAnalyzeDWGVision] analysing DWG tiles for', filename, '| tiles:', capped.length, '(capped≤3)');
  const kb = knowledgeBaseHints();

  const tileFiles = capped.map(b64 => ({ type: 'image/png', b64 }));
  ...
  const scheduleHint = converterResult?._schedule_markdown
    ? `\nSERVER SCHEDULE-FIRST EXTRACT (prefer these numbers; do not invent others):\n${String(converterResult._schedule_markdown).slice(0, 20000)}\n`
    : '';
...
- Prefer schedule QTY column; never count from plan view.
