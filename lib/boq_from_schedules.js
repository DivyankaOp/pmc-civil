...
function buildBoqFromSchedules(extracted, opts = {}) {
  ...
  const globalHeightM = opts.defaultStoreyHeightM
    ?? opts.pedestalHeightM
    ?? meta.pedestal_height_m
    ?? undefined;
  const pccThk = opts.pccThicknessM
    ?? opts.defaultPccThicknessM
    ?? (meta.pcc_thickness_mm ? meta.pcc_thickness_mm / 1000 : undefined);
  const notFound = [];
  const boq = [];
  let sr = 1;
  ...
  for (const row of schedules.columns || []) {
    ...
    if (qty == null || qty <= 0) {
      notFound.push(`Column ${row.mark}: qty not found — ASK USER`);
      continue;
    }
    ...
    colRcc += vol;
    colCount += qty;
    const hSrc = opts.columnHeights?.[row.mark] || row.height_m
      ? 'user/schedule'
      : (meta.pedestal_height_m ? 'printed on drawing' : 'confirmed');
    const hNote = `size ${row.size_mm} × H ${heightM}m (${hSrc}) × qty ${qty}${row.source?.includes('layout') ? ' [layout count]' : ''}`;
    boq.push(item(sr++, 'PART A — STRUCTURE',
      `RCC Columns/Pedestal ${row.mark} (${row.size_mm})`, 'cum', vol, rcc,
      row.source || 'calculated-from-schedule', hNote,
      row.source?.includes('layout') ? 'medium' : 'high'));
  ...
  for (const row of schedules.footings || []) {
    ...
    if (!depthM) {
      ...
      boq.push(item(sr++, 'PART A — STRUCTURE',
        ...
        row.source || 'calculated-from-schedule',
        `${row.rcc_size_mm || row.pcc_size_mm} × depth ${row.depth_mm}mm × qty ${qty}${/layout/i.test(row.source || '') ? ' [from linked layout counts]' : ''}`,
        /split/i.test(row.source || '') ? 'low' : 'high'));
...
function formatBoqMarkdown(boqResult, opts = {}) {
  const parts = [];
  const status = opts.status || (boqResult?.not_found?.length ? 'DRAFT' : 'FINAL');
  parts.push(`### Bill of Quantities — **${status}** (from drawing reading)`);
  if (!boqResult?.boq?.length) {
    parts.push('_No BOQ lines yet — finish drawing reading / confirm missing cells._');
  ...
  parts.push('| Sr | Description | Unit | Qty | Rate | Amount | Calc / source |');
  parts.push('|---:|---|---|---:|---:|---:|---|');
  for (const i of boqResult.boq) {
    const calc = (i.calc_note || i.source || '').replace(/\|/g, '/');
    parts.push(`| ${i.sr} | ${i.description} | ${i.unit} | ${i.qty} | ${i.rate} | ${i.amount} | ${calc} |`);
  }
  const tot = boqResult.cost_summary?.civil_total_inr || 0;
  parts.push(`\n**Civil total (${status}): ₹${tot.toLocaleString('en-IN')}** (${boqResult.cost_summary?.civil_total_lacs || 0} Lacs)`);
  if (status !== 'FINAL') {
    parts.push('\n_⚠️ DRAFT total — missing confirmations hone ke baad FINAL hoga._');
  }
  if (boqResult.not_found?.length) {
    parts.push('\n#### Not found / skipped');
    for (const n of boqResult.not_found) parts.push(`- ${n}`);
  }
  parts.push('\n_Rates: data/Rates.json (Gujarat DSR). Quantities = drawing reading only._');
  return parts.join('\n');
}
