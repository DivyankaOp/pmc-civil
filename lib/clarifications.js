...
function buildClarifications({
  ...
  const wantsBoq = /boq|quantity|pcc|rcc|cum|estimate|volume|calculate/i.test(q);
  ...
  for (const row of ftgs) {
    ...
    if (row.qty == null) {
      pushQ(questions, `ftg_qty_${mark}`,
        `Footing **${mark}** ki QTY kitni hai? (plan count assume nahi)`,
        'Qty missing', 'footing.qty');
    }
  ...
  if (wantsBoq && (ftgs.length || /pcc/i.test(q))) {
    if (!/pcc[^a-z0-9]{0,12}(\d{2,3})\s*mm|(\d{2,3})\s*mm[^a-z0-9]{0,12}pcc/i.test(text)) {
