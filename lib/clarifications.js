...
function buildClarifications({
  ...
  const missingQty = ftgs.filter(r => r.qty == null || r.qty <= 0);
  ...
  // Pedestal height once when columns ready (needed for full structure BOQ)
  if (wantsBoq && cols.some(c => c.qty > 0 && c.size_mm && !/not found/i.test(c.size_mm))) {
    const hasH = cols.some(c => c.height_m || c.floor_height_m) || schedules?.meta?.pedestal_height_m;
    if (!hasH) {
      pushQ(questions, 'col_height_all',
        'Pedestal/column height up to plinth (m) drawing/section pe kya printed hai? Sab same ho to ek number, warna `C1=…, C2=…`.',
        'Height needed for pedestal RCC — not assumed');
    }
  }
    ...
    for (const row of cols) {
      ...
      if (row.qty == null || row.qty <= 0) {
        pushQ(questions, `col_qty_${mark}`,
          `Column **${mark}** ki QTY schedule/plan pe kitni marks hain?`,
          'Qty missing', 'column.qty');
      }
    }
  }

  if (wantsBoq && (ftgs.length || /pcc/i.test(q))) {
    // Mix ratio alone is NOT thickness — only printed mm counts
    const pccKnown = !!schedules?.meta?.pcc_thickness_mm
      || /P\.?C\.?C[^.\n]{0,60}?\d{2,3}\s*mm/i.test(text)
      || /\d{2,3}\s*mm[^.\n]{0,40}?P\.?C\.?C/i.test(text);
    if (!pccKnown) {
      pushQ(questions, 'pcc_thickness',
        'PCC thickness (mm) drawing me kya printed hai? (jo sheet pe likha hai woh type karo — assume nahi).',
...
function mergeUserAnswers(base, userText, clarifications) {
  ...
  for (const [id, val] of Object.entries(answers)) {
    ...
    if (colH) {
      if (colH[1] === 'all') {
        const pairs = [...String(val).matchAll(/([Cc]\d{1,2})\s*[=:]\s*(\d+(?:\.\d+)?)/g)];
        if (pairs.length) {
          opts.columnHeights = opts.columnHeights || {};
          for (const p of pairs) opts.columnHeights[p[1].toUpperCase()] = Number(p[2]);
        } else {
          const n = Number(String(val).replace(/[^\d.]/g, ''));
          if (n > 0) {
            opts.defaultStoreyHeightM = n > 20 ? n / 1000 : n;
            opts.pedestalHeightM = opts.defaultStoreyHeightM;
          }
        }
      } else {
        const n = Number(String(val).replace(/[^\d.]/g, ''));
        opts.columnHeights = opts.columnHeights || {};
        opts.columnHeights[colH[1]] = n > 20 ? n / 1000 : n;
      }
    }
    if (id === 'ftg_qty_confirm_split' && schedules.schedules?.footings) {
      ...
      if (/^(ok|yes|confirm|sahi|theek)/i.test(val.trim())) {
        for (const row of schedules.schedules.footings) {
          if (row.qty) row.source = 'user-confirmed';
        }
      }
    }
