...
function extractSchedules(textOrLines, opts = {}) {
  ...
  // Scanned / odd layouts: OCR splits cells across lines — fill empty schedule types
  const needOcr = (schedules.footings || []).length < 1
    || (schedules.columns || []).length < 1
    || totalRows < 2
    || quality === 'poor';
  if (needOcr) {
    try {
      const { enrichSchedulesFromOcr } = require('./ocr_footing_schedule');
