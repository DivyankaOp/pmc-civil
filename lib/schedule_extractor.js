...
// Many CAD offices use different titles — match broadly, never one fixed wording.
const SCHEDULE_HEADERS = [
  { type: 'columns',  re: /schedule\s*of\s*column|scheduleofcolumn|column\s*schedule|r\.?c\.?c\.?\s*column\s*schedule|pedestal\s*schedule|col\.?\s*sch(?:edule)?/i },
  { type: 'footings', re: /schedule\s*of\s*footing|scheduleoffooting|footing\s*schedule|foundation\s*schedule|schedule\s*of\s*foundation|raft\s*schedule|ftg\.?\s*sch(?:edule)?/i },
  { type: 'beams',    re: /schedule\s*of\s*beam|beam\s*schedule|r\.?c\.?c\.?\s*beam\s*schedule/i },
...
function parseBarSpec(raw) {
  ...
  // Stirrup spacing / c/c is NOT main bar count
  if (/c\s*\/\s*c|c\/c/i.test(s)) return '';
  ...
  if (m) {
    ...
    // Reject OCR junk like 104-125 (spacing) or 10-150 as "bars"
    if (nos > 40 || dia > 40) return '';
    if (nos >= 8 && dia >= 100) return ''; // 10-150 style spacing
    return m[0].replace(/[Oo]/g, 'Ø').slice(0, 40);
  }
  return '';
}
...
function trailingQty(tokens, line) {
  ...
  for (let i = tokens.length - 1; i >= 0; i--) {
    ...
    const n = Number(t);
    // Reject grid dims / spacing masquerading as qty
...
/** Column/pedestal sizes mis-read as footing rows under FOOTING header */
function isJunkFootingRow(row) {
  if (!row) return true;
  const mark = String(row.mark || '');
  if (/not\s*found/i.test(mark) || !mark) return true;
  if (!/^F/i.test(mark) && !/FTG|FOOT/i.test(mark)) {
    // Allow only clear footing marks — CM2 / C1 etc. are not footings
    if (/^C/i.test(mark) || /^CM/i.test(mark) || /^P\d/i.test(mark)) return true;
  }
  const size = String(row.rcc_size_mm || row.pcc_size_mm || '');
  if (/not\s*found/i.test(size) && row.depth_mm == null) return true;
  const m = size.match(/(\d+)\s*[xX×]\s*(\d+)/);
  if (!m) return row.depth_mm == null;
  const a = Number(m[1]);
  const b = Number(m[2]);
  const max = Math.max(a, b);
  const min = Math.min(a, b);
  // Pedestal / column: both sides small, or long thin base-plate without depth
  if (max <= 900 && min <= 900 && row.depth_mm == null) return true;
  if (min < 500 && max > 900 && row.depth_mm == null) return true;
  return false;
}
...
function extractSchedules(textOrLines, opts = {}) {
  ...
  // Drop false footing rows (column/pedestal sizes OCR'd under FOOTING header)
  schedules.footings = (schedules.footings || []).filter(row => !isJunkFootingRow(row));
  schedules.columns = (schedules.columns || []).filter(row => {
    if (!row?.size_mm || /not found/i.test(row.size_mm)) return false;
    if (/not found/i.test(row.mark || '')) return false;
    return true;
  });

  const meta = extractMeta(lines);
  let totalRows = Object.values(schedules).reduce((n, a) => n + (a?.length || 0), 0);
  let usefulChars = scheduleTextChars;
  let quality = totalRows >= 2 || usefulChars >= 200 ? 'good' : usefulChars >= 80 ? 'weak' : 'poor';
  let ocrEnriched = false;

  // Scanned / odd layouts: OCR splits cells — fill empty OR replace junk columns
  let needOcr = true;
  try {
    const { enrichSchedulesFromOcr, columnsLookJunk } = require('./ocr_footing_schedule');
    needOcr = (schedules.footings || []).length < 1
      || (schedules.columns || []).length < 1
      || columnsLookJunk(schedules.columns)
      || totalRows < 2
      || quality === 'poor';
    if (needOcr) {
      const enriched = enrichSchedulesFromOcr({
        schedules,
        meta,
      ...
      Object.assign(schedules, enriched.schedules);
      Object.assign(meta, enriched.meta || {});
      schedules.footings = (schedules.footings || []).filter(row => !isJunkFootingRow(row));
      schedules.columns = (schedules.columns || []).map(c => ({
        ...c,
        qty: (c.qty != null && c.qty > 80) ? null : c.qty,
        main_bars: /c\s*\/\s*c/i.test(c.main_bars || '') ? 'not found in drawing' : c.main_bars,
      })).filter(row => {
        if (!row?.size_mm || /not found/i.test(row.size_mm)) return false;
        if (/not found/i.test(row.mark || '')) return false;
        return true;
      });
      totalRows = Object.values(schedules).reduce((n, a) => n + (a?.length || 0), 0);
      usefulChars = enriched.schedule_text_chars;
      quality = enriched.quality || quality;
      ocrEnriched = true;
    }
  } catch (e) {
    console.warn('[schedule_extractor] OCR footing enrich failed:', e.message);
  }
