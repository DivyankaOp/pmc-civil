...
function runScheduleFirstLocal(text, opts = {}) {
  ...
  const action = String(opts.action || '').toLowerCase().trim() || null;
  const autoBoq = opts.autoBoq !== false && action !== 'read' && action !== 'study';
  const defaultQ = action === 'read'
    ? 'Read this drawing only — type, title, schedules, levels, dims, notes. Do not prepare BOQ.'
    : action === 'study'
      ? 'Study this civil drawing — educational walkthrough with how-to-read and quiz. Do not prepare BOQ.'
      : 'Prepare quantity takeoff / BOQ from this drawing — read schedules, calculate quantities, ask missing values one-by-one.';

  // Read / study never run trade takeoff agents
  const scope = (action === 'read' || action === 'study') ? null : (opts.scope || null);

  const qa = answerFromDrawing({
    text,
    filename: opts.filename || '',
    question: opts.question || defaultQ,
    hints: opts.hints || typeInfo.secondary_types || [],
    boqOpts: opts.boqOpts || {},
    extracted,
    scope,
    polylines: opts.polylines || [],
    action: action || (autoBoq ? 'calculate' : null),
    autoBoq,
  });

  const boqResult = qa.meta?.boqResult || qa.boqResult || buildBoqFromSchedules(extracted, opts.boqOpts || {});
  boqResult.drawing_type = opts.drawing_type || boqResult.drawing_type || typeInfo.drawing_type;

  const markdown = qa.markdown || buildReportMarkdown(qa.extracted || extracted, boqResult);
  const needsVision = extracted.quality === 'poor' && String(text || '').length < 400;
  return {
    extracted: qa.extracted || extracted,
    boqResult,
    markdown,
    needsVision,
    ...
    typeInfo,
    qa,
    measure: qa.measure || qa.meta?.measure || null,
    geotech: qa.geotech || qa.meta?.geotech || null,
    earthworks: qa.earthworks || qa.meta?.earthworks || null,
    paving: qa.paving || qa.meta?.paving || null,
    groundworks: qa.groundworks || qa.meta?.groundworks || null,
    groundModel: qa.groundModel || qa.meta?.groundModel || null,
    qaChecklist: qa.qaChecklist || qa.meta?.qaChecklist || null,
    scope,
  };
}
