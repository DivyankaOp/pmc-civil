...
function buildReportMarkdown(extracted, boqResult, extraNotes = []) {
  ...
  parts.push('', '> Pipeline: **read → calculate → BOQ** (Civils.ai-style). Tokens only if extract weak.');
  return parts.join('\n');
}
...
function runScheduleFirstLocal(text, opts = {}) {
  ...
  const qa = answerFromDrawing({
    text,
    filename: opts.filename || '',
    question: opts.question || 'Read this construction drawing fully like Civils.ai: sheet type, title, schedules, levels, dims, notes; calculate quantities with formulas; then BOQ. Never invent.',
...
function applyUserClarifications({ text, extracted, clarifications, userText, filename, question, hints, boqOpts }) {
  ...
  let markdown = qa.markdown;
  if (!remaining.length) {
    markdown = markdown.replace(/## ❓ Please confirm[\s\S]*?(?=\n> Local|\n> \*\*Waiting|$)/, '')
      + '\n\n> ✅ User answers applied — BOQ rebuilt without invented values.';
  }
