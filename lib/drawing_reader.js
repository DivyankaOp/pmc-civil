...
function readDrawingFully({
  ...
  if (intent === 'footing_calc') {
    ...
    if (boqResult?.boq?.length) {
      ...
      parts.push('', '_Volumes abhi nahi bane — qty / PCC thickness confirm karo._');
    }
    ...
    parts.push('', '> BOQ chahiye ho to explicitly **BOQ** likh kar poocho.');
  ...
  return {
    answeredLocally,
    needsClaude: String(text || '').trim().length < 80 && clarifications.questions.length === 0,
