...
/**
 * Intent: answer ONLY what user asked.
 * - read (default): drawing reading, NO BOQ
 * - footing_calc: PCC/RCC footing only
 * - schedule: tables only
 * - levels: levels only
 * - boq: full BOQ (explicit)
 * - qa: short focused answer
 */
function detectIntent(question) {
  const t = String(question || '').toLowerCase().trim();
  if (/\bboq\b|bill of quant|full\s*takeoff|complete\s*boq|prepare\s*boq/i.test(t)) return 'boq';
  if (/pcc|rcc|footing.*(calc|volume|cum|qty)|calculate.*(pcc|rcc|footing)|footing.*(pcc|rcc)/i.test(t)) {
    return 'footing_calc';
  }
  if (/^levels?$|level|plinth|ffl|height.*floor|storey/i.test(t) && !/boq|pcc|rcc/i.test(t)) return 'levels';
  if (/schedule|footing size|column size|table/i.test(t) && !/boq|calculate|volume/i.test(t)) return 'schedule';
  // Default upload prompts / empty → READ ONLY (no BOQ)
  if (!t
    || /read this|drawing read|padho|study|samjhao|analyze|analyse|sheet type|title block|civils\.ai-style: read|do not (prepare )?boq|without boq/i.test(t)) {
    return 'read';
  }
  return 'qa';
}
...
function readDrawingFully({
  ...
  const needsCalc = intent === 'boq' || intent === 'footing_calc';
  let boqResult = null;
  if (needsCalc && (inv.total_rows >= 1
    || /FOUNDATION|COLUMN|INDUSTRIAL|FLOOR/i.test(typeInfo.drawing_type))) {
    boqResult = filterBoqForIntent(buildBoqFromSchedules(schedules, boqOpts), intent);
  ...
  const clarifyQuestion = intent === 'read' ? 'read drawing only'
    : intent === 'footing_calc' ? (question || 'calculate pcc rcc footing')
      : intent === 'boq' ? (question || 'prepare boq')
        : question;
  ...
  if (intent === 'footing_calc') {
    ...
    if (boqResult?.boq?.length) {
      ...
      parts.push('', '_Volumes abhi nahi bane — neeche **ek-ek question** se confirm karenge (qty / size / PCC)._');
      if (!inv.footings) {
        parts.push('_Schedule OCR weak hai — aap footing lines type kar sakte ho: `F1 2600x1800 900 12`_');
      }
    }
    ...
    // Questions asked one-by-one in UI — keep first only in markdown
    ...
    parts.push('# BOQ (explicitly requested)');
