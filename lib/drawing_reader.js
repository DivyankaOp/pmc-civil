...
function detectIntent(question, action) {
  ...
  // Default upload / takeoff / analyze → draft quantity takeoff (auto BOQ)
  if (!t
    || /takeoff|take-off|measure|quantit|material\s*list|auto\s*excel|prepare\s*takeoff|upload|analyze|analyse|auto.?detect/i.test(t)) {
    return 'takeoff';
  }
  if (/read this|drawing read|padho|sheet type|title block/i.test(t)) {
    return 'read';
  }
  return 'qa';
}
...
function readDrawingFully({
  ...
  // Product: autoBoq / calculate upload never skips calc via read/study keywords alone
  const effectiveAction = autoBoq && forced !== 'read' && forced !== 'study'
    ? (action || 'calculate')
    : action;
  const skipTrade = !autoBoq && (
    forced === 'read' || forced === 'study'
    || String(effectiveAction || '').toLowerCase() === 'read'
    || String(effectiveAction || '').toLowerCase() === 'study'
  );
  ...
  const intent = autoBoq && !['read', 'study', 'footing_calc', 'geotech', 'earthworks', 'paving', 'groundworks'].includes(forced)
    ? 'takeoff'
    : detectIntent(effectiveQuestion, effectiveAction);
  ...
  if (!boqResult && needsCalc) {
    // Always attempt DRAFT BOQ on auto takeoff — even if schedule rows weak
    const calcIntent = intent === 'footing_calc' ? 'footing_calc' : 'boq';
