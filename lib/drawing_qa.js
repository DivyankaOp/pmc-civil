...
function answerFromDrawing(opts) {
  // Default = read only unless caller sets takeoff/calculate action or question
  const question = opts.question
    || 'Read this drawing only — sheet type, title, schedules, levels, dims, notes. Do not prepare BOQ unless asked.';
  return readDrawingFully({ ...opts, question, action: opts.action || null });
}
