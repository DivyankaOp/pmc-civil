...
function answerFromDrawing(opts) {
  // Default intent = full read + takeoff (Civils.ai), not BOQ-only
  const question = opts.question
    || 'Read this construction drawing fully like Civils.ai: identify sheet type, title block, schedules/tables, levels, dimensions, notes; calculate quantities with formulas; then prepare BOQ. Do not invent values.';
  return readDrawingFully({ ...opts, question });
}
