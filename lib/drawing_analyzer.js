...
`.trim();
  ...
  // ── Build context + schedule-first local BOQ ──────────────────
  ...
  const localSched = extractSchedules(context);
  if (localSched.quality === 'good' && localSched.total_schedule_rows >= 2) {
    const localBoq = buildBoqFromSchedules(localSched);
    console.log(`[PMC] Schedule-first local BOQ: ${localBoq.boq.length} items (skipping Claude invent)`);
    return {
      ...localBoq,
      legend: [],
      area_statement: localBoq.area_statement || { total_bua_sqmt: 0, floor_wise: [] },
      cv_analysis: cvData || {},
      schedule_markdown: formatSchedulesMarkdown(localSched),
      boq_markdown: formatBoqMarkdown(localBoq),
      pipeline_info: { reading_sources: sources, claude_mode: 'SKIPPED — schedule-first local', schedule_quality: localSched.quality },
    };
  }

  // Weak schedules → one Claude text-only call (still zero image tokens)
  const kb = knowledgeBaseHints();
  const raw = await callClaudeTextOnly(
    `${formatSchedulesMarkdown(localSched)}\n\n${context}`,
    filename,
    kb
  );
  ...
  boq = validateBOQ(boq);
  // Prefer local schedule_data if Claude omitted it
  if (!boq.schedule_data || !Object.keys(boq.schedule_data).length) {
    boq.schedule_data = {
      columns: localSched.schedules.columns,
      footings: localSched.schedules.footings,
      doors: localSched.schedules.doors,
      windows: localSched.schedules.windows,
    };
  }
  ...
  return {
    project_name:     boq.project_name || localSched.meta.project_name || 'CIVIL PROJECT',
    drawing_type:     boq.drawing_type || 'GENERAL',
    drawing_no:       boq.drawing_no   || localSched.meta.drawing_no || '',
    scale:            boq.scale        || localSched.meta.scale || '',
    legend:           boq.legend       || [],
    ...
    pipeline_info:    { reading_sources: sources, claude_mode: 'TEXT ONLY — zero image tokens', schedule_quality: localSched.quality }
  };
}
