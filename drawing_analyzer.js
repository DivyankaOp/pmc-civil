...
// ── RATES loaded from rates.json (no hardcoded numbers here) ────────
// Rates are the single source of truth in Rates.json at project root.
// Edit that file to update rates — no code change required.
const RATES = (() => {
  const out = {};
  try {
    ...
    for (const category of Object.values(raw)) {
      if (typeof category === 'object' && !Array.isArray(category)) {
        for (const [key, val] of Object.entries(category)) {
          if (val && typeof val.rate === 'number') out[key] = val.rate;
        }
      }
    }
  } catch (e) {
    console.warn('rates.json not loaded in drawing_analyzer:', e.message);
  }
  return out;
})();
